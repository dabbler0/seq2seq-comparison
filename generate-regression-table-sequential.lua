...
function main()
  ...
  for name_A, encoding_A in pairs(encodings) do
    ...
    for name_B, encoding_B in pairs(encodings) do
      ...
      local concatenated = torch.cat(encoding_A, encoding_B, 2)

      local mses = {}

      for i=encoding_A:size(2),concatenated:size(2)-1 do
        print('Computing dimension', i)
        local mm_matrix = torch.gels(concatenated:narrow(2, i + 1, 1), concatenated:narrow(2, 1, i))
        local mse = torch.mean(torch.pow(
          torch.csub(torch.mm(concatenated:narrow(2, 1, i), mm_matrix:narrow(1, 1, i)), concatenated:narrow(2, i + 1, 1)),
          2
        ))
        table.insert(mses, mse)
      end

      -- Extract MSE into a Lua table
      -- and put it into the big mapping table
      mapping_table[name_B] = mse
    end
